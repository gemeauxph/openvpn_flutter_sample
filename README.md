...
Open `VPNExtension > PacketTunnelProvider.swift` and copy-paste the script from [PacketTunnelProvider.swift](https://raw.githubusercontent.com/nizwar/oneconnect_flutter/master/example/ios/VPNExtension/PacketTunnelProvider.swift).
...
void initState() {
    ...
    openvpn.initialize(
        groupIdentifier: "GROUP_IDENTIFIER", ///Example 'group.top.oneconnect.vpn'
...
2. Add the following to your files in the `android` folder (special thanks to https://github.com/nizwar/oneconnect_flutter/issues/10). Otherwise, the connection may not be established in some cases and will silently report "disconnected" when trying to connect. This is likely related to some symbols being stripped by Google Play.
...
* [oneconnect_flutter](https://github.com/nizwar/oneconnect_flutter/blob/master/LICENSE) for this plugin
